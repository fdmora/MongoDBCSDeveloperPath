...
var firstDocument = accountsCollection
   .Find(a => a.AccountId == "MDB773154309")
   .FirstOrDefault();
...
// TODO: Complete the `Find()` query so that it returns the first document in the `accounts` collection with `AccountId` of `MDB773154309` using LINQ below:
var firstDocument = await accountsCollection.FindAsync(account => account.AccountId == "MDB773154309").Result.FirstOrDefaultAsync();
